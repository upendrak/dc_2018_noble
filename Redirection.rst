...
-  Practice searching for characters or patterns in a text file using
   the ``grep`` command
-  Learn about output redirection
-  Explore how to use the pipe (``|``) character to chain together
   commands
...
| Suppose we want to see how many reads in our file have really bad,
  with 10 consecutive Ns
| Let’s search for the string NNNNNNNNNN in file ``SRR098026.fastq`` in
  the ``untrimmed_fastq`` folder:
...
We get back a lot of lines. What is we want to see the whole fastq
record for each of these read. We can use the ``-B`` argument for grep to
return the matched line plus one before (-B 1) and two lines after (-A
2). Since each record is four lines and the last second is the sequence,
this should give the whole record.
...
1) Search for the sequence GNATNACCACTTCC in SRR098026.fastq. In
   addition to finding the sequence, have your search also return the
...
nothing happened. But type ``ls``. You should have a new file called
``bad_reads.txt``. Take a look at it and see if it has what you think it
should.

If we use ``>>``, it will append to rather than overwrite a file. This can
be useful for saving more than one search, for example:
...
Finally, let’s use the new tools in our kit and a few new ones to
example our SRA metadata file.
...
That’s only the first line but it is a lot to take in. ‘cut’ is a
program that will extract columns in tab-delimited files. It is a very
good command to know. Lets look at just the first four columns in the
header using the ``\|`` redirect and ‘cut’
...
``-f 1-4`` means to cut the first four fields (columns). The
LibraryLayout_s column looks promising. Let’s look at some data for just
that column.
...
We can see that there are (at least) two categories, SINGLE and PAIRED.
We want to search all entries in this column for just PAIRED and count
the number of hits.
...
We can use some new tools ‘sort’ and ‘uniq’ to extract more information.
For example, cut the third column, remove the header and sort the
values. The ``-v`` option for grep means return all lines that DO NOT
match.
...
This returns a sorted list (too long to show here) of PAIRED and SINGLE
values. Now we can use ‘uniq’ with the ‘-c’ flag to count the different
categories.
...
3) Sort the metadata file by PAIRED/SINGLE and save to a new file We can
   use if ‘-k’ option for sort to specify which column to sort on. Note
   that this does something similar to cut’s ‘-f’.

.. code:: bash

   $ sort -k3 SraRunTable.txt > SraRunTable_sorted_by_layout.txt

4) Extract only paired end records into a new file Do we know PAIRED
   only occurs in column 4? WE know there are only two in the file, so
   let’s check.
...
-  Software Carpentry tutorial - `The Unix
   shell <http://software-carpentry.org/v4/shell/index.html>`__
-  The shell handout - `Command
   Reference <http://files.fosswire.com/2007/08/fwunixref.pdf>`__
-  `explainshell.com <http://explainshell.com>`__
-  http://tldp.org/HOWTO/Bash-Prog-Intro-HOWTO.html
-  man bash
-  Google - if you don’t know how to do something, try Googling it.
   Other people have probably had the same question.
-  Learn by doing. There’s no real other way to learn this than by
   trying it out. Write your next paper in nano (really emacs or vi),
   open pdfs from the command line, automate something you don’t really
   need to automate.
